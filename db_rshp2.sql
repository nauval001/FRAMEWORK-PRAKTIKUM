...
-- Dumping data for table db_rshp2.jenis_hewan: ~6 rows (approximately)
...
-- Dumping data for table db_rshp2.pemilik: ~0 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `dokter` (
  ...
  KEY `fk_dokter_user_idx` (`id_user`),
  CONSTRAINT `fk_dokter_user` FOREIGN KEY (`id_user`) REFERENCES `user` (`iduser`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Dumping for Tabel Perawat
CREATE TABLE IF NOT EXISTS `perawat` (
  ...
  KEY `fk_perawat_user_idx` (`id_user`),
  CONSTRAINT `fk_perawat_user` FOREIGN KEY (`id_user`) REFERENCES `user` (`iduser`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- === DATA DOKTER ===
-- Memasukkan data profil untuk user 'dokters' (iduser=11) ke tabel dokter
INSERT INTO `dokter` (`alamat`, `no_hp`, `bidang_dokter`, `jenis_kelamin`, `id_user`) 
VALUES 
('Jl. Kesehatan No. 10', '08123456789', 'Dokter Hewan Umum', 'L', 11);

-- Data Perawat
INSERT INTO `user` (`nama`, `email`, `password`) 
VALUES 
('Suster Siti', 'siti@mail.com', '$2y$10$dummyPasswordHashForExampleOnly...'); 

INSERT INTO `role_user` (`iduser`, `idrole`, `status`) 
VALUES 
((SELECT iduser FROM user WHERE email='siti@mail.com'), 3, 1);

INSERT INTO `perawat` (`alamat`, `no_hp`, `jenis_kelamin`, `pendidikan`, `id_user`) 
VALUES 
('Jl. Melati No. 5', '08987654321', 'P', 'D3 Keperawatan Hewan', (SELECT iduser FROM user WHERE email='siti@mail.com'));
...
-- Dumping data for table db_rshp2.role_user: ~0 rows (approximately)
...
-- Dumping data for table db_rshp2.user: ~0 rows (approximately)
INSERT INTO `user` (`iduser`, `nama`, `email`, `password`) VALUES
	...
	(10, 'Nauval PDR', 'nauvalpdr@mail.com', '$2y$10$yoS2LGPLhsVKD6Ja8LauwOZoKfQTNAGRb6gnM4oDRZDujaCGm18Ti'),
